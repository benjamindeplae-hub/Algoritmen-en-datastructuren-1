...
(define-library (linked-sorted-list)
  (export new from-scheme-list sorted-list? empty? full? length
          find! find-ternary! delete! add! peek
    ...
    (define (find-ternary! slst key)
      ...
      (let ternary-search
        ((left 0)
         (right (- leng 1)))
        (if (<= left right)
            (let* ((a (+ left (quotient (- right left) 3)))  ; First division point
                   (b (+ left (* 2 (quotient (- right left) 3))))) ; Second division point
              (cond
                ((==? (vector-ref vect a) key) (current! slst a)) ; Key matches the first third
                ((==? (vector-ref vect b) key) (current! slst b)) ; Key matches the second third
                ((<<? key (vector-ref vect a)) ; Key is in the first third
                 (ternary-search left (- a 1)))
                ((<<? (vector-ref vect b) key) ; Key is in the third third
                 (ternary-search (+ b 1) right))
                (else ; Key is in the middle third
                 (ternary-search (+ a 1) (- b 1)))))
            (current! slst -1))) ; Key not found
      slst)
    ; End
