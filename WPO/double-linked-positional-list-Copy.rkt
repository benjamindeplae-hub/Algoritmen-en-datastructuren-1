...
(define-library (double-linked-positional-list)
  ...
  (begin   
    ...
    ; Added ex 4
    (define (make-list-node v p n)
      (vector 'listnode v p n))

    (define (list-node? x)
      (and (vector? x)
           (= (vector-length x) 4)
           (eq? (vector-ref x 0) 'list-node)))
    
    (define (list-node-val n)
      (vector-ref n 1))

    (define (list-node-val! n v)
      (vector-set! n 1 v))

    (define (list-node-prev n)
      (vector-ref n 2))

    (define (list-node-prev! n prev)
      (vector-set! n 2 prev))

    (define (list-node-next n)
      (vector-ref n 3))

    (define (list-node-next! n next)
      (vector-set! n 3 next))
    ; End ex 4
